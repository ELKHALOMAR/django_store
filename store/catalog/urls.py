...
from store.catalog.views import *
...
urlpatterns = [
    ...
    url(r'^manufacturers/(?P<slug>[\w-]+)/$', ManufacturerProductsView.as_view(), {'page_num': 1}, name='catalog_manufacturer'),
    url(r'^manufacturers/(?P<slug>[\w-]+)/(?P<page_num>\d+)/$', ManufacturerProductsView.as_view(), name='catalog_manufacturer'),
